...
```
In case of any disaster, the first hour is crucial. The actions taken in that
greatly increases the chances of survival in the aftermath. But, Citizens usually come to  
know about the incoming diaster via sirens (present near beaches, public  
places etc.), news, social networking etc., and often, *from the disaster itself.*
```

### Proposed Solution

```
1. In case of earthquake (also floods generated by an earthquake ex. Tsunami)
...
If the threshold is too low, end users might get irritated of frequent notifications.
	...
	(Most people think twice before uninstalling but not before switching off notifications.)
```
